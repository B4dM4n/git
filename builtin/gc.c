...
static int systemd_timer_write_service_template(const char *exec_path)
{
	...
	const char *unit;
	char *local_service_name = xstrfmt(SYSTEMD_UNIT_FORMAT, "", "service");
	int global_service = 0;
	struct stat st;

	repo_config_get_bool(the_repository, "maintenance.global-systemd-service", &global_service);
